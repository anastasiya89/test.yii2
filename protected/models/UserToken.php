...
class UserToken extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, token, date_expiry, date_created, date_edited', 'required'),
			array('user_id', 'numerical', 'integerOnly'=>true),
			array('token', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, token, date_expiry, date_created, date_edited', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('date_expiry',$this->date_expiry,true);
